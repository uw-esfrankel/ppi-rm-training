...
DATASET_TO_KEYS = {
    "nvidia/HelpSteer": [
        "average_rating",
        "average_rating_no_verbosity",
        "average_rating_no_verbosity_no_complexity",
        "goodness_score",
    ],
    "nvidia/HelpSteer2": [
        "average_rating",
    ...
    "lmarena-ai/arena-human-preference-55k": ["winner"],
}
...
def compute_average_rating(row):
    ratings = [
        row[k]
        for k in ["helpfulness", "correctness", "coherence", "complexity", "verbosity"]
    ]
...
def compute_average_rating_no_verbosity(row):
    ratings = [
        row[k] for k in ["helpfulness", "correctness", "coherence", "complexity"]
    ]
...
DATASET_TO_COMPUTE_FUNCTIONS = {
    ...
    "lmarena-ai/arena-human-preference-55k": lambda row: row,
}
...
def group_and_sort(dataset, dataset_key, dataset_name):
    ...
    for row in dataset:
        ...
        elif dataset_name == "lmarena-ai/arena-human-preference-55k":
            if row["winner_tie"] != 1:
                grouped_data[prompt] = [
                    {"response": row["response_a"], "winner": row["winner_model_a"]},
                    {"response": row["response_b"], "winner": row["winner_model_b"]},
                ]
            else:
                continue
        else:
    ...
    for prompt in grouped_data:
        grouped_data[prompt].sort(
            key=lambda x: x[dataset_key], reverse=dataset_name != "berkeley-nest/Nectar"
        )
    return grouped_data
...
def load_and_process_dataset(dataset_name):
    if (
        ...
        or dataset_name == "nvidia/HelpSteer2"
        or dataset_name == "berkeley-nest/Nectar"
        or dataset_name == "lmarena-ai/arena-human-preference-55k"
    ):
        ...
        dataset_a = dataset_a.remove_columns(
            ["models", "correct_answers", "incorrect_answers"]
        )
        ...
        dataset_a = dataset_a.filter(
            lambda x: x["instruction"] not in contaminated_prompts
        )
        print(
        ...
        def remove_partially_annotated(dataset_row: dict) -> dict:
            ...
            for completion in dataset_row["completions"]:
                if not all(
                    ...
                    for aspect in completion["annotations"].keys()
                ):
                    continue
                ratings = []
                for aspect in [
...
def binarize_dataset(dataset, dataset_name, dataset_keys, compute_function):
    ...
    if data_name == "arena-human-preference-55k":
        data_name = "ChatbotArena55k"
    ...
    for dataset_key in dataset_keys:
        ...
        for prompt, samples in grouped_data.items():
            ...
            remaining_samples = [
                s for s in samples[1:] if s[dataset_key] != chosen_score
            ]
        ...
        binarized_dataset_split.push_to_hub(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--recreate_all",
        action="store_true",
        help="Delete all existing datasets and re-process them",
    )
