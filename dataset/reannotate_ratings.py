...
def main(args):
    ...
    vllm_manager = VLLMManager(
        ...
        port=args.port,
        seed=args.seed,
        vllm_process_outdir=dirname,
    )

    assert slurm_task_id < slurm_num_tasks, (
        f"slurm_task_id {slurm_task_id} is greater than or equal to slurm_num_tasks {slurm_num_tasks}"
    )
...
if __name__ == "__main__":
    ...
    # dataset arguments
    parser.add_argument(
        "--dataset_name", choices=["nectar", "uf"], type=str, required=True
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Number of parallel process to spawn",
    )
    parser.add_argument(
        "--num_data_points",
        "-n",
        type=int,
        help="Number of datapoints to annotate per process",
    )
    parser.add_argument(
        "--k_val",
        "-k",
        type=int,
        help="K for K-wise: gives the number of responses per prompt",
    )
    parser.add_argument(
        "--explain", default=False, action=argparse.BooleanOptionalAction
    )
    parser.add_argument(
        "--do-pairwise", default=False, action=argparse.BooleanOptionalAction
    )
    parser.add_argument(
        "--shuffle", default=False, action=argparse.BooleanOptionalAction
    )
